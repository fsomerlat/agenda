<?php	
	...
	class Profissional extends CI_Controller {
		...
		public function __construct() {
			...
			$this->load->model("profissional_model","prof");
		}
		
		public function FormProfissional() {
			
			$this->load->view('profissional', self::FormAcaoProf());
		}
		
		public function service_profissional() {
			
			header("Content-Type: application/json");
			print_r($this->prof->getJSON());
		}	
		...
		public function FormAcaoProf () {
			...
			if($this->form_validation->run() ==  FALSE): // INICIO CADASTRA E ATUALIZA
				...
				if($valor_campo["acao"] == "cadastrar"): 
					...
					$this->prof->cadastraProfissional( 
						...
						redirect("profissional/FormProfissional?panel=9",set_msg("<div class='alert alert-success'>Registro inserido com sucesso !</div>"));
				...
				if($valor_campo["acao"] == "atualizar"):	
					...
					$this->prof->atualizaProfissional(
						...
						redirect("profissional/FormProfissional",set_msg("<div class='alert alert-success'>Registro atualizado com sucesso !</div>"));	
			...
			if(isset($_GET["id"])):
				...
				if($_GET["acao"] == "editar"): 
					...
					$getInfoProf = $this->prof->getInfoProfissional($id);
				...
				if($_GET["acao"] == "excluir"): 
				    ...
				    $this->prof->excluiProfissional(addslashes($id));
